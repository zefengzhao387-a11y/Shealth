      ...
      >
        ...
        <Suspense fallback={null}>
          <Physics gravity={gravity} timeStep={isMobile ? 1 / 30 : 1 / 60}>
            <Band isMobile={isMobile} scenePosition={scenePosition} />
          </Physics>
        </Suspense>
...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false, scenePosition = [0, 4, 0] }) {
  const band = useRef()
  const fixed = useRef()
  const j1 = useRef()
  const j2 = useRef()
  ...
  const lineResolution = [
    Math.max(64, Math.round(size.width * (isMobile ? 1.5 : 2))),
    Math.max(64, Math.round(size.height * (isMobile ? 1.5 : 2))),
  ]
